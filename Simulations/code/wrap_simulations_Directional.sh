...
if [[ -d "$folder" ]] && [[ -f $folder/Rep_1_OPT_${_arg_optimum}_ES_${_arg_selectcoeff}_f_${_arg_frequency}.out.gz ]] && [[ "$_arg_overwrite" == "off" ]]
then 
    die "$_arg_directory"" is not empty. Remove it before running the script again. Or use the --overwrite option."
elif [[ -d "$folder" ]] && [[ -f $folder/Rep_${_arg_replicate}_OPT_${_arg_optimum}_ES_${_arg_selectcoeff}_f_${_arg_frequency}.out.gz ]]
then
    echo "Remove existing results from $folder and overwrite..."
    rm $folder/Rep_${_arg_replicate}_OPT_${_arg_optimum}_ES_${_arg_selectcoeff}_f_${_arg_frequency}*
else
...
do
    echo "Replicate $i"
    ./slim -t -M -d P="'""$folder""'" -d R="${i}" -d S="${_arg_selectcoeff}" -d O="${_arg_optimum}" -d f="${_arg_frequency}" Directional.polygen.slim >$folder/Rep_${i}_OPT_${_arg_optimum}_ES_${_arg_selectcoeff}_f_${_arg_frequency}.out 2>$folder/Rep_${i}_OPT_${_arg_optimum}_ES_${_arg_selectcoeff}_f_${_arg_frequency}.err  
